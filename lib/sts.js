...
var STS = exports.STS = function STS(config) {
  ...
  this.__defineGetter__('credentials', function() {
  ...
  this.__defineSetter__('credentials', function(resp){
    credentials = {
...
STS.prototype.getSessionToken = function(cb) {
  ...
  var url = URL.format({host:defaults.host, protocol: "https", query : this.buildParams() });
  ...
  request(url, function (err, resp, body) {
    ...
    self.parser.parseString(body, function (err, result) {
      ...
      if(resp.statusCode != 200) return cb(new Error(result.Error.Code + ": " + result.Error.Message));
...
STS.prototype.buildParams = function() {
  var params = {
  ...
  var signature = crypto.createHmac('sha256', this.secretAccessKey).update(toSign).digest("base64");
