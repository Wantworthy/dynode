...
Client.prototype._request = function(action, options, cb) {
  ...
  operation.attempt(function(currentAttempt) {
    ...
    self.sts.getSessionToken(function(err, credentials) {
      if(err) return cb(err);
      ...
      request({
        ...
        if(resp.statusCode != 200) {
          var err = new AmazonError({type : json["__type"], message: json["message"], statusCode: resp.statusCode});
          if(err.retry && operation.retry(err)) return; // check to see if should retry request
