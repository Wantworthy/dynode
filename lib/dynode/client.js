...
Client.prototype.putItem = function(tableName, item, cb){
  var request = {TableName: tableName, Item: this.formatAttributes(item)};
  this._request("PutItem", request, cb);
};

Client.prototype.formatAttributes = function(attrs) {
  var result = {};
  
  _.each(attrs, function(value, key) {
    var attr = {};
    attr[typeIndicator(value)] = stringify(value);
...
function stringify(value) {
  if(_.isArray(value)) {
    ...
    return _.isNumber(value) ? Number(value).toString() : value.toString();
  }
};
